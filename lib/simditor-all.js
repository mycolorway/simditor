    ...
    Formatter.prototype.format = function($el) {
      ...
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        ...
        if ($node.is('br')) {
          ...
          $node.remove();
        } else if (this.editor.util.isBlockNode(node) || $node.is('img:not([data-non-image])')) {
          if ($node.is('li')) {
  ...
  ImageButton = (function(_super) {
    ...
    function ImageButton(editor) {
      ...
      this.editor.body.on('click', 'img:not[data-non-image]', function(e) {
        ...
        if ($img.hasClass('active')) {
          ...
          _this.editor.body.find('img.active').removeClass('active');
          $img.addClass('active');
          _this.popover.show($img);
          range = _this.editor.selection.getRange();
          range.selectNode($img[0]);
          _this.editor.selection.selectRange(range);
          _this.editor.triiger('selectionchanged');
        }
        return false;
      });
      this.editor.on('selectionchanged.image', function() {
        ...
        if ($contents.length === 1 && $contents.is('img:not[data-non-image]')) {
          return _this.popover.show($contents);
        } else {
    ...
    ImageButton.prototype._initUploader = function() {
      ...
      this.editor.uploader.on('beforeupload', function(e, file) {
        ...
        return _this.editor.uploader.readImageFile(file.obj, function(img) {
          ...
          return _this.loadImage($img, src, function() {
            var $progress, imgPosition;
            _this.popover.refresh();
            _this.popover.srcEl.val('正在上传...');
            imgPosition = $img.position();
            $progress = $('<div class="upload-progress"><span></span></div>').css({
              top: imgPosition.top(),
              left: imgPosition.left(),
              width: $img.width(),
              height: $img.height()
            }).appendTo(_this.editor.wrapper);
            if (!_this.editor.uploader.html5) {
              $progress.addClass('loading');
            }
            $img.addClass('uploading');
            return $img.data('progress', $progress);
          });
        });
      });
      this.editor.uploader.on('uploadprogress', function(e, file, loaded, total) {
        ...
        return $progress.find("span").text(percent);
      });
