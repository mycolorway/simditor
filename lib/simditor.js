...
Selection = (function(superClass) {
  ...
  Selection.prototype.endNodes = function() {
    var endNodes;
    if (this._range) {
      this._endNodes || (this._endNodes = this._range.collapsed ? this.startNodes() : (endNodes = $(range.endContainer).parentsUntil(this.editor.body).get(), endNodes.unshift(range.endContainer), $(endNodes)));
    }
...
Keystroke = (function(superClass) {
  ...
  Keystroke.prototype._init = function() {
    ...
    this.editor.inputManager.addKeystrokeHandler('13', 'li', (function(_this) {
      return function(e, $node) {
        var $cloneNode, isClosestBlock, isEmptyNode, listEl, newBlockEl, newListEl;
        $cloneNode = $node.clone();
        $cloneNode.find('ul, ol').remove();
        isEmptyNode = _this.editor.util.isEmptyNode($cloneNode);
        isClosestBlock = $node.is(_this.editor.util.closestBlockEl());
        if (!(isEmptyNode && isClosestBlock)) {
          return;
        }
        listEl = $node.parent();
    ...
    this.editor.inputManager.addKeystrokeHandler('13', 'blockquote', (function(_this) {
      return function(e, $node) {
        var $closestBlock, isEmptyNode, isLastBlock, range;
        $closestBlock = _this.editor.util.closestBlockEl();
        isLastBlock = !$closestBlock.next().length;
        isEmptyNode = _this.editor.util.isEmptyNode($closestBlock);
        if (!($closestBlock.is('p') && isLastBlock && isEmptyNode)) {
          return;
        }
...
Util = (function(superClass) {
  ...
  Util.prototype.traverseUp = function(callback, node) {
    var j, len, n, nodes, range, result, results;
    if (node == null) {
      range = this.editor.selection.range();
      node = range != null ? range.commonAncestorContainer : void 0;
    }
...
TitleButton = (function(superClass) {
  ...
  TitleButton.prototype.setActive = function(active, param) {
    var ref;
    if (param == null) {
      param = (ref = this.node[0]) != null ? ref.tagName.toLowerCase() : void 0;
    }
  ...
  BlockquoteButton.prototype.command = function() {
    ...
    clearCache = function() {
      if (nodeCache.length > 0) {
        $("<" + this.htmlTag + "/>").insertBefore(nodeCache[0]).append(nodeCache);
