...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define('simditor', ["jquery","simple-module","simple-hotkeys"], function ($, SimpleModule, simpleHotkeys, simpleUploader) {
    ...
    // like Node.
    module.exports = factory(require("jquery"),require("simple-module"),require("simple-hotkeys"));
  } else {
    root['Simditor'] = factory(jQuery,SimpleModule,simple.hotkeys);
  }
...
Simditor.i18n = {
  ...
  'en-US': {
    ...
    'italic': 'Italic',
    'link': 'Insert Link',
    'linkTextTitle': 'Add Link',
    'linkText': 'Link Name',
    'linkUrl': 'Link Url',
    'linkUrlSubmit': 'Confirm',
    'linkTarget': 'Target',
...
LinkButton = (function(superClass) {
  ...
  LinkButton.prototype._status = function() {
    ...
    if (this.active && ((ref = this.node) != null ? ref.is('[rel=nofollow]') : void 0)) {
      return this.setLink();
    } else if (this.active && !this.editor.selection.rangeAtEndOf(this.node)) {
      this.popover.show(this.node);
      if (this.node.attr('href') !== "") {
        return this.popoverStatus = 3;
      } else if (this.withLinkText) {
        return this.popoverStatus = 2;
      } else {
        return this.popoverStatus = 1;
      }
    } else {
      return this.popover.hide();
    }
  ...
  LinkButton.prototype.command = function() {
    var $contents, $newBlock, linkText, range, txtNode;
    range = this.editor.selection.range();
    if (this.active) {
      ...
      if (this.editor.selection.blockNodes().length > 0) {
        range.insertNode(this.$link[0]);
      } else {
        $newBlock = $('<p/>').append(this.$link);
      ...
      this.popover.one('popovershow', (function(_this) {
        return function() {
          if (linkText) {
            ...
            _this.popover.urlEl[0].select();
            return _this.withLinkText = true;
          } else {
            _this.popover.textEl.focus();
            _this.popover.textEl[0].select();
            return _this.withLinkText = false;
          }
        };
      })(this));
    }
  ...
  LinkButton.prototype.setLink = function() {
    var href, text;
    text = this.node.text();
    href = this.node.attr('href');
    if (href !== text) {
      if (text && !/https?:\/\/|^\//ig.test(text)) {
        text = 'http://' + text;
      }
      return this.node.attr('href', text);
    }
  };
...
LinkPopover = (function(superClass) {
  ...
  LinkPopover.prototype.render = function() {
    ...
    this.textEl.on('keyup', (function(_this) {
      return function(e) {
        ...
        _this.editor.inputManager.throttledValueChanged();
        return _this.checkButtonStatus();
      };
    })(this));
    ...
    this.confirm.on('click', (function(_this) {
      return function(e) {
        var val;
        if (e.which === 13) {
          return;
    ...
    return this.close.on('click', (function(_this) {
      return function(e) {
        var txtNode;
        _this.editor.inputManager.throttledValueChanged();
        switch (_this.button.popoverStatus) {
          case 1:
            _this.button.node.remove();
            break;
          case 2:
  ...
  LinkPopover.prototype.checkButtonStatus = function(val1, val2) {
    val1 || (val1 = this.textEl.val());
    val2 || (val2 = this.urlEl.val());
    if (!(/https?:\/\/|^\//ig.test(val2) || !val2)) {
      val2 = 'http://' + val2;
    }
    return this.confirm[val1 && val2 ? 'removeClass' : 'addClass']('disabled');
  };
  ...
  LinkPopover.prototype.show = function() {
    ...
    val1 = this.target.text();
    this.textEl.val(val1);
    this.val = this.target.attr('href');
    this.urlEl.val(this.val);
    this.checkButtonStatus(val1, this.val);
...
UploadButton = (function(superClass) {
  ...
  UploadButton.prototype.insertImage = function(src, name) {
    var $img;
  ...
  UploadButton.prototype.command = function() {
    return this.editor.trigger('uploadclick');
  };
