(function() {
  ...
  ImagePopover = (function(_super) {
    ...
    ImagePopover.prototype.render = function() {
      ...
      this.el.find('.image-size').on('keyup', function(e) {
        var height, inputEl, value, width;
        inputEl = $(e.currentTarget);
        value = inputEl.val() * 1;
        if (!($.isNumeric(value) || value < 0)) {
          return;
        }
        if (inputEl.is(_this.widthEl)) {
          height = _this.height * value / _this.width;
          _this.heightEl.val(height);
        } else {
          width = _this.width * value / _this.height;
          _this.widthEl.val(width);
        }
        _this.target.css({
          width: width || value,
          height: height || value
        });
        _this.el.data('popover').refresh();
        return inputEl.blur().focus();
      });
      this.el.find('.btn-restore').on('click', function(e) {
        var size;
        size = _this.target.data('image-size').split(",") || [_this.width, _this.height];
        _this.target.css({
          width: size[0] * 1,
          height: size[1] * 1
        });
        _this.el.data('popover').refresh();
        _this.widthEl.val(size[0]);
        return _this.heightEl.val(size[1]);
      });
