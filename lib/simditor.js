(function() {
  ...
  ImageButton = (function(_super) {
    ...
    ImageButton.prototype.createImage = function() {
      var $breakedEl, $endBlock, $img, $startBlock, caret, endNode, range, startNode;
      range = this.editor.selection.getRange();
      if (!range) {
        caret = this.editor.inputManager.lastCaretPosition;
        this.editor.undoManager.caretPosition(caret);
        range = this.editor.selection.getRange();
      }
      startNode = range.startContainer;
      endNode = range.endContainer;
