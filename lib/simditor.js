(function() {
  ...
  ImagePopover = (function(_super) {
    ...
    ImagePopover.prototype.render = function() {
      ...
      this.input = this.el.find('input[name=upload_file]');
    ...
    ImagePopover.prototype._initUploader = function() {
      ...
      this.editor.uploader.on('beforeupload', function(e, file) {
        ...
        _this.editor.uploader.readImageFile(file.obj, function(img) {
          if (img) {
            return _this.button.loadImage($img, img.src, function(success) {
              if (!success) {
                return;
              }
              _this.refresh();
        ...
        $bar = _this.target.find(".simditor-image-progress-bar");
        $bar.show();
        $img.css('opacity', '0.8');
      ...
      this.editor.uploader.on('uploadsuccess', function(e, file, result) {
        ...
        $img = _this.target.find("img");
        $img.css('opacity', '1');
        _this.target.find(".simditor-image-progress-bar").hide().removeClass('hint');
        _this.srcEl.val(result.file_path);
        return _this.button.loadImage($img, result.file_path, function(success) {
          if (!success) {
            return;
          }
          return _this.editor.trigger('valuechanged');
      ...
      this.editor.uploader.on('uploadcomplete', function(e, file, result) {
        if (!file.inlineImage) {
      ...
      return this.editor.uploader.on('uploaderror', function(e, file, xhr) {
        ...
        if (xhr.responseText) {
          return result = $.parseJSON(xhr.responseText);
        }
      });
    };
